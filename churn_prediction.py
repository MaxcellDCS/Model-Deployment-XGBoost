...
def main():
    ...
    Surname = st.text_input("Surname") # Done
    ...
    data = {'Surname': Surname, 'CreditScore':int(CreditScore), 'Geography': Geography, 'Gender': Gender, 'Age': int(Age),
    ...
    df = pd.DataFrame([list(data.values())], columns=['Surname','CreditScore','Geography','Gender','Age','Tenure','Balance',
                                                      'NumOfProducts','HasCrCard','IsActiveMember','EstimatedSalary'])

    df = df.replace(gender_encode)
    cat_subs = df[['Geography']]
    cat_enc_geo=pd.DataFrame(oneHot_encode_geo.transform(cat_subs).toarray(),columns=oneHot_encode_geo.get_feature_names_out())
